...
class ProductController extends Controller
{
    ...
    public function product_wip_complete(Request $req){
        ...
        if($amount <= $wip->product_amount){
            ...
            // $proyek     = DB::table('proyek')->where("warehouse_id", $warehouse_id)->select("proyek_id")->first();
            if(!empty($shelf)){
    ...
    public function product_stock(Request $req){
        ...
        if(!empty($amount)){
            if(!empty($req->shelf)){
                ...
                $totalStockIn   = DB::table('stock')->where([["warehouse_id", $warehouse_id], ["product_id", $product_id], ["shelf_id", $shelf], ["proyek_id", $proyek], ["type", 1]])->sum("product_amount");
                $totalStockOut  = DB::table('stock')->where([["warehouse_id", $warehouse_id], ["product_id", $product_id], ["shelf_id", $shelf], ["proyek_id", $proyek], ["type", 0]])->sum("product_amount");
                $availableStock = $totalStockIn-$totalStockOut;
    ...
    public function product_stock_history(Request $req){
        ...
        $history = DB::table('stock')
                    ...
                    ->leftJoin("proyek", "stock.proyek_id", "=", "proyek.proyek_id")
                    ->leftJoin("users", "stock.user_id", "=", "users.id")
                    ->select("stock.*", "products.product_code", "products.product_name", "products.satuan", "shelf.shelf_name", "proyek.proyek_name", "users.name");
        ...
        if(!empty($search)){
            $history = $history->orWhere([["products.product_code", "LIKE", "%".$search."%"], ["products.warehouse_id", $warehouse_id]])
                        ...
                        ->orWhere([["proyek.proyek_name", "LIKE", "%".$search."%"], ["products.warehouse_id", $warehouse_id]]);
        } else {
    ...
    public function proyek(Request $req){
        $product_id = $req->product_id;
        ...
        if($req->format == "json"){
            if(!empty($product_id)){
                $proyek = $proyek->join("stock", "proyek.proyek_id", "stock.proyek_id")
                            ->where([["stock.product_id", $product_id], ["stock.warehouse_id", $warehouse_id]])->groupBy("proyek_id");
                $result = [];
                $proyek = $proyek->select("proyek.*", "stock.product_amount")->get();
                foreach($proyek as $s){
                    $totalStockIn   = DB::table('stock')->where([["stock.warehouse_id", $warehouse_id], ["product_id", $product_id], ["proyek_id", $s->proyek_id], ["type", 1]])->sum("product_amount");
                    $totalStockOut  = DB::table('stock')->where([["stock.warehouse_id", $warehouse_id], ["product_id", $product_id], ["proyek_id", $s->proyek_id], ["type", 0]])->sum("product_amount");
                    $availableStock = $totalStockIn-$totalStockOut;
                    if($availableStock > 0){
                        $s->product_amount = $availableStock;
                        $result[] = $s;
                ...
                $result = $proyek->select("proyek.*")->get();
            }
            return response()->json($result);
        } else {
            $proyek = $proyek->where("warehouse_id", $warehouse_id)->paginate(50);
            if(Auth::user()->role == 0){
                $warehouse = $this->getWarehouse();
                return View::make("proyek")->with(compact("proyek", "warehouse"));
            } else {
                abort(403);
            }
        }
    ...
    public function proyek_save(Request $req){
        ...
        // ],
        // [
        //     'proyek_name.required'     => 'proyek Name belum diisi!',
        // ]);

        $data = [
            "warehouse_id"    => $warehouse_id,
            "proyek_name"      => $req->proyek_name
        ];

        if(empty($proyek_id)){
            $add = DB::table('proyek')->insertGetId($data);

            if($add){
                $req->session()->flash('success', "Proyek baru berhasil ditambahkan.");
            } else {
                $req->session()->flash('error', "Proyek baru gagal ditambahkan!");
            }
        } else {
            $edit = DB::table('proyek')->where([["proyek_id", $proyek_id], ["warehouse_id", $warehouse_id]])->update($data);

            if($edit){
                $req->session()->flash('success', "Proyek berhasil diubah.");
            } else {
                $req->session()->flash('error', "Proyek gagal diubah!");
            }
        }
    ...
    public function proyek_delete(Request $req){
        if(Session::has('selected_warehouse_id')){
            $warehouse_id = Session::get('selected_warehouse_id');
        } else {
            $warehouse_id = DB::table('warehouse')->first()->warehouse_id;
        ...
        $del = DB::table('proyek')->where([["proyek_id", $req->delete_id], ["warehouse_id", $warehouse_id]])->delete();

        if($del){
            DB::table('stock')->where([["proyek_id", $req->delete_id], ["warehouse_id", $warehouse_id]])->delete();
            $req->session()->flash('success', "Proyek berhasil dihapus.");
        } else {
            $req->session()->flash('error', "Proyek gagal dihapus!");
        }
